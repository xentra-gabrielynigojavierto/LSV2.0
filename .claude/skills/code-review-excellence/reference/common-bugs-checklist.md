...
```csharp
// ❌ Bad: resource leaked if exception is thrown
var conn = new SqlConnection(connectionString);
conn.Open();
DoWork(conn);
conn.Dispose();  // never reached on exception

// ✅ Good: using guarantees disposal
...
```csharp
// ❌ Bad: exceptions crash the process silently
async void LoadAsync() { await FetchAsync(); }

// ✅ Good: exceptions are observable
async Task LoadAsync() { await FetchAsync(); }

// ✅ Acceptable: event handlers only
button.Click += async (s, e) => await LoadAsync();
```
...
```csharp
// ❌ Bad: enumerates twice — may hit DB / produce different results
...
```csharp
// ❌ Bad: subscriber keeps publisher alive indefinitely
public class Subscriber
{
    public Subscriber(Publisher pub) { pub.DataReceived += OnData; }
    void OnData(object? s, EventArgs e) { }
    // OnData never unsubscribed — GC can't collect Subscriber
}
...
public sealed class Subscriber : IDisposable
{
    ...
    public Subscriber(Publisher pub) { _pub = pub; _pub.DataReceived += OnData; }
    void OnData(object? s, EventArgs e) { }
    public void Dispose() => _pub.DataReceived -= OnData;
}
```

### Struct Mutability Surprise

```csharp
// ❌ Bad: mutates a copy, not the original
struct Point { public int X; public int Y; }
var points = new List<Point> { new Point { X = 1, Y = 2 } };
points[0].X = 99;  // compile error in newer C# — or silently mutates a copy

// ✅ Good: use immutable structs or replace via index
points[0] = points[0] with { X = 99 };  // C# 10+ record struct
```
...
// ✅ Good: observe exceptions or log failures
_ = Task.Run(async () =>
{
    try { await RiskyOperationAsync(); }
    catch (Exception ex) { _logger.LogError(ex, "Background task failed"); }
});
```

### C# Common Mistakes Checklist
- [ ] All `IDisposable` / `IAsyncDisposable` resources use `using`
- [ ] No `async void` except event handlers
- [ ] No `.Result` or `.Wait()` blocking on async code
- [ ] `IEnumerable` not enumerated more than once without materializing
- [ ] Event handlers are unsubscribed (`-=`) in `Dispose`
- [ ] Structs are treated as value types (mutations affect copies, not originals)
- [ ] Fire-and-forget tasks log or handle exceptions
- [ ] `CancellationToken` is accepted and propagated through async call chains
- [ ] Nullable reference types enabled; no unjustified `!` suppressions
