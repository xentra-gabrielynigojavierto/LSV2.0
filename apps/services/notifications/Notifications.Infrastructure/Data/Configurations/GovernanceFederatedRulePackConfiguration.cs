...
internal sealed class GovernanceFederatedRulePackConfiguration
    : IEntityTypeConfiguration<GovernanceFederatedRulePack>
{
    public void Configure(EntityTypeBuilder<GovernanceFederatedRulePack> builder)
    {
        builder.ToTable("ntf_GovernanceFederatedRulePacks");
        ...
        builder.HasIndex(x => new { x.ChannelType, x.Enabled, x.Priority })
               .HasDatabaseName("IX_ntf_GovFedPack_Channel_Enabled_Priority");
        builder.HasIndex(x => new { x.RulePackId, x.ChannelType })
               .HasDatabaseName("IX_ntf_GovFedPack_Pack_Channel");
        builder.HasIndex(x => new { x.TenantId, x.ChannelType, x.Enabled })
               .HasDatabaseName("IX_ntf_GovFedPack_Tenant_Channel_Enabled");
        builder.HasIndex(x => new { x.FederationGroup, x.Enabled })
               .HasDatabaseName("IX_ntf_GovFedPack_FedGroup_Enabled");
        builder.HasIndex(x => new { x.EffectiveFrom, x.EffectiveTo })
               .HasDatabaseName("IX_ntf_GovFedPack_EffectiveWindow");
    }
}
