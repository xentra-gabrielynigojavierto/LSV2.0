import type { RelationshipTypeItem } from '@/types/control-center';

interface RelationshipTypeTableProps {
  relTypes: RelationshipTypeItem[];
}

export function RelationshipTypeTable({ relTypes }: RelationshipTypeTableProps) {
  if (relTypes.length === 0) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-10 text-center">
        <p className="text-sm text-gray-400">No relationship types found.</p>
      </div>
    );
  }
            ...
            {relTypes.map(rt => (
              <tr key={rt.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-gray-500 max-w-xs">
                  {rt.description || <span className="text-gray-300">—</span>}
                </td>
                <td className="px-4 py-3">
                  <StatusBadge active={rt.isActive} />
                </td>
                <td className="px-4 py-3 text-xs text-gray-400">
                  {new Date(rt.createdAtUtc).toLocaleDateString()}
                </td>
              </tr>
            ))}
          </tbody>
