...
export interface SelectedFile {
  /** Client-side stable key for list rendering. */
  id:   string;
  file: File;
}
...
/**
 * FileAttachmentPicker — drag-drop + click-to-browse file picker.
 *
 * Validates file type and size client-side. Exposes `SelectedFile[]` to the
 * parent; the parent is responsible for uploading via the BFF proxy after the
 * form's primary action (create ticket / add comment) succeeds.
 */
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <span className="text-gray-500">
          {dragOver ? 'Drop files here' : 'Attach files'}
        </span>
        <span className="ml-auto text-xs text-gray-400 hidden sm:inline">
          PDF, images, Office docs, CSV · max 20 MB
        </span>
      </div>
      ...
      {files.length > 0 && (
        <ul className="space-y-1" aria-label="Attached files">
          {files.map(sf => (
            <li
              key={sf.id}
              className="flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 border border-gray-200 rounded-md text-xs"
            >
...
/**
 * Uploads selected files to the ticket attachment endpoint via the BFF proxy.
 * Files are uploaded sequentially to avoid overwhelming the server.
 * Non-fatal: individual upload failures are collected and returned.
 */
  ...
  for (const sf of files) {
    ...
    try {
      const res = await fetch(
        `/api/support/api/tickets/${encodeURIComponent(ticketId)}/attachments/upload`,
        { method: 'POST', body: fd },
      );
      if (!res.ok) {
