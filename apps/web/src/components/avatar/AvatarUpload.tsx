...
/**
 * AvatarUpload — client component for the Profile page.
 *
 * Shows the current avatar (or initials fallback) and lets the user
 * pick a new image or remove the existing one.
 * On success it calls router.refresh() so the server session re-renders
 * with the updated avatarDocumentId (including the TopBar).
 */
export function AvatarUpload({ avatarDocumentId, initials }: Props) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Validate client-side: images only, max 5 MB
